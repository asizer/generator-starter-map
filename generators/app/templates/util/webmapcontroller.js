  ...
  function(declare, lang, topic, on,
    ...
    return declare([], {
      ...
      constructor: function(options) {
        if (options.portal && options.portal.portalUrl) {
          arcgisUtils.arcgisUrl = options.portal.portalUrl + 'content/items/';
        }
      ...
      initMap: function(options) {
        var webmapOptions = {
          usePopupManager: true // otherwise, layers not visible on load will never have popups
        };
        arcgisUtils.createMap(options.mapSetup.webmapId, options.mapDiv, webmapOptions)
          .then(lang.hitch(this, this.handleMapCreation));
      },

      handleMapCreation: function(response) {
        this.map = response.map;
        this.attachEventListeners();
        on(this.map, 'load', lang.partial(this.initialPublish, response));
      },
      ...
      initialPublish: function(response) {
        topic.publish('map/loaded', {
          webmapResponse: response,
          map: response.map,
          basemaps: response.itemInfo.itemData.baseMap.baseMapLayers,
